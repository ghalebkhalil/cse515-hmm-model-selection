...
- or symmetric KL divergence between Gaussian emissions.
...
1. Prepare data matrix \( X \in \mathbb{R}^{T \times d} \)
2. Choose a grid of candidate hidden state counts
3. Fit the best model for each \( K \) using multiple restarts
4. Compute AIC and BIC
5. Inspect which values of \( K \) minimize the criteria
6. Compare fitted models across restarts
7. Align states
8. Measure redundancy and instability
...
# Candidate number of hidden states
K_grid = [1, 2, 3, 4, 5]
